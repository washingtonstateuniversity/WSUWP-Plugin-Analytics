( function( $, window, analytics ) {
	...
	/**
	 * Determine if this is a mobile view using the same definition as the
	 * WSU Spine - less than 990px.
	 *
	 * @returns boolean
	 */
	function is_mobile() {
	...
	if ( analytics.app.ga_code !== false ) {
		...
		if ( is_mobile() ) {
			spine_type = "spine-mobile";
		} else {
			spine_type = "spine-full";
		}
		...
		rendered_accounts = jQuery.merge( rendered_accounts, [ {
			...
			settings:{
				...
				dimension:[
					...
					{ "name":"dimension8", "val": spine_type }                        // The type of Spine - mobile or full.
				],
